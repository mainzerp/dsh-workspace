...
export function apply(ctx, config = {}) {
    ...
    const peakWindows = (config.peakWindows ?? [[540, 720], [840, 1_080]]).map(pair => [pair[0], pair[1]]);
    ...
    ctx.effect(() => webServer.register({
        ...
        async handler(req, res) {
            ...
            if (config.allowRemote !== true && !isLoopback(req)) {
                sendJson(res, 403, { status: 403, reason: 'FORBIDDEN', message: '更新检查默认仅允许本机读取', fields: [], requestId }, requestId);
                return;
            }
            try {
    ...
    ctx.effect(() => webServer.register({
        ...
        async handler(req, res) {
            const requestId = typeof req.headers['x-request-id'] === 'string' && req.headers['x-request-id'].length > 0 ? req.headers['x-request-id'] : randomUUID();
            if (!requirePost(req, res, requestId, '更新'))
                return;
            try {
    ...
    ctx.effect(() => webServer.register({
        ...
        async handler(req, res) {
            const requestId = typeof req.headers['x-request-id'] === 'string' && req.headers['x-request-id'].length > 0 ? req.headers['x-request-id'] : randomUUID();
            if (!requirePost(req, res, requestId, '重启'))
                return;
