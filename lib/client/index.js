...
function DrawerExpandToggle() {
    ...
    if (!collapsed)
        return null;
    return _jsx("button", { type: "button", className: "hui-drawer-toggle", "aria-label": t.expandDrawer, title: t.expandDrawer, onClick: () => writeDrawerCollapsed(false), children: _jsx(PanelRightOpen, { size: 16 }) });
}
...
function HarnessSummary({ wide, sessions }) {
    ...
    return _jsxs("div", { className: `hui-summary${wide ? '' : ' rail'}`, children: [_jsxs("div", { className: "hui-summary-main", "aria-label": t.balanceStatus, title: snapshot?.balance.error, children: [wide ? _jsxs("span", { className: "hui-content", children: [_jsxs("span", { className: "hui-period", "data-period": period, title: period === 'idle' ? t.offPeakBillingPeriod : t.peakBillingPeriod, children: [_jsx("i", {}), period === 'idle' ? t.offPeak : t.peak] }), _jsx("span", { className: "hui-balance-line", children: _jsx("b", { "data-tone": balanceTone, children: balanceValue }) })] }) : _jsx("span", { className: "hui-period", "data-period": period, title: period === 'idle' ? t.offPeakBillingPeriod : t.peakBillingPeriod, children: _jsx("i", {}) }), wide && scheduleBar !== null ? _jsxs("div", { className: "hui-daybar", role: "img", "aria-label": t.scheduleBarHint, title: t.scheduleBarHint, children: [scheduleBar.segments.map((segment, index) => _jsx("span", { className: "hui-daybar-peak", style: { left: `${segment.start * 100}%`, width: `${(segment.end - segment.start) * 100}%` } }, index)), _jsx("span", { className: "hui-daybar-marker", style: { left: `${scheduleBar.markerFraction * 100}%` } })] }) : null] }), _jsx(ProjectDrawer, { sessionId: sessionId, cwd: cwd })] });
}
...
.hui-drawer{box-sizing:border-box}.hui-drawer[data-collapsed]{display:none}:where(.hui-drawer-toggle){position:fixed;top:56px;right:10px;z-index:40;display:grid;grid-auto-flow:column;width:32px;height:32px;padding:0;place-items:center;border:1px solid var(--dsw-alias-border-l1,#c7ccd5);border-radius:8px;background:var(--dsw-alias-bg-base,#fff);color:var(--dsw-alias-label-secondary,#6c768a);box-shadow:0 1px 3px rgb(15 23 42/.12);cursor:pointer;pointer-events:auto}:where(.hui-drawer-toggle:hover){background:var(--dsw-alias-interactive-bg-hover,#eef0f4);color:var(--dsw-alias-label-primary,#172033)}
`;
const FLAT_STYLES = `
...
`;
const DAYBAR_STYLES = `
.hui-daybar{position:relative;flex:0 0 100%;width:100%;min-width:0;height:9px;margin-top:3px;overflow:visible}
.hui-daybar::before{content:'';position:absolute;left:0;right:0;top:3px;height:3px;border-radius:2px;background:var(--dsw-alias-label-success,#16895a)}
.hui-daybar-peak{position:absolute;top:3px;height:3px;background:var(--dsw-alias-label-error,#d94a4a)}
...
export async function apply(ctx) {
    ...
    ctx.effect(() => {
        ...
        style.id = STYLE_ID;
        style.textContent = STYLES + STATUS_STYLES + BASE_STYLES + FLAT_STYLES + EDITOR_STYLES + GRAPH_STYLES + DAYBAR_STYLES;
