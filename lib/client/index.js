...
function commitTime(timestamp) {
    ...
    if (elapsed < 86_400)
        return `${Math.floor(elapsed / 3_600)}h ago`;
    if (elapsed < 7 * 86_400)
...
function TerminalPanel({ sessionId, cwd }) {
    ...
    useEffect(() => {
        ...
        void request('open').then(async (response) => {
            ...
            if (data.id === undefined)
                throw new Error('No terminal session id returned');
            ...
            if (!disposed)
                term.write(`\x1b[31mFailed to open terminal: ${reason instanceof Error ? reason.message : String(reason)}\x1b[0m`);
        });
    ...
    return _jsxs("div", { className: "hui-terminal", children: [_jsxs("div", { className: "hui-term-toolbar", children: [_jsxs("span", { children: [_jsx(Terminal, { size: 13 }), " Terminal"] }), _jsx("span", { className: "hui-term-actions", children: ready ? _jsx("button", { type: "button", title: "Clear output", "aria-label": "Clear output", onClick: clear, children: _jsx(Eraser, { size: 13 }) }) : null })] }), _jsx("div", { className: "hui-term-screen", ref: containerRef })] });
}
function ProjectDrawer({ sessionId, cwd }) {
    ...
    const open = useCallback((path, kind) => {
        ...
        void fetch(`${endpoint}?${params.toString()}`).then(async (response) => {
            ...
            if ('content' in data) {
                ...
                setPreview((data.diff || 'No textual diff to display for this change') + (data.truncated ? '\n\n…preview truncated' : ''));
    ...
    return _jsxs("aside", { className: "hui-drawer", "aria-label": "Project preview", style: effectiveWidth === null ? undefined : { width: effectiveWidth }, children: [_jsx("div", { className: "hui-resizer", "aria-hidden": "true", onPointerDown: startResize }), _jsx("header", { className: "hui-titlebar", children: _jsxs("div", { children: [_jsx("span", { className: "hui-vscode-mark", children: _jsx(Blocks, { size: 14 }) }), _jsx("span", { title: snapshot?.rootPath, children: snapshot?.rootPath ?? 'Loading current project…' })] }) }), error ? _jsxs("div", { className: "hui-error", children: [error, _jsx("button", { type: "button", onClick: () => setError(null), "aria-label": "Dismiss error", title: "Dismiss", children: _jsx(X, { size: 12 }) })] }) : null, _jsxs("div", { className: "hui-workbench", "data-preview": (selected !== null && mode !== 'terminal') || undefined, children: [_jsxs("nav", { className: "hui-activity", "aria-label": "Project views", children: [_jsx("button", { type: "button", "aria-label": "Explorer", title: "Explorer", "data-active": mode === 'files' || undefined, onClick: () => setMode('files'), children: _jsx(FolderTree, { size: 20 }) }), _jsxs("button", { type: "button", "aria-label": "Source Control", title: "Source Control", "data-active": mode === 'changes' || undefined, onClick: () => setMode('changes'), children: [_jsx(GitBranch, { size: 20 }), snapshot?.changes.length ? _jsx("b", { children: snapshot.changes.length }) : null] })] }), _jsxs("section", { className: "hui-explorer", children: [_jsxs("header", { children: [_jsx("strong", { children: mode === 'files' ? 'Explorer' : mode === 'changes' ? 'Source Control' : 'Terminal' }), mode !== 'terminal' ? _jsx("button", { type: "button", onClick: refresh, "aria-label": "Refresh", children: _jsx(RefreshCw, { size: 15 }) }) : null] }), mode === 'terminal' ? _jsx(TerminalPanel, { sessionId: sessionId, cwd: cwd }) : mode === 'files' ? _jsxs(_Fragment, { children: [_jsxs("div", { className: "hui-section-title", children: [_jsx(ChevronDown, { size: 14 }), _jsx("b", { children: snapshot?.rootName?.toUpperCase() ?? 'PROJECT' })] }), _jsxs("div", { className: "hui-tree", role: "tree", children: [visibleEntries.map(entry => entry.kind === 'directory'
                                                ...
                                                : _jsx("div", { role: "treeitem", className: "hui-tree-row", "data-selected": selected === entry.path || undefined, style: { paddingLeft: 20 + Math.min(entry.depth, 12) * 13 }, title: entry.path, children: _jsxs("button", { type: "button", className: "hui-tree-open", onClick: () => open(entry.path, 'file'), children: [_jsx(FileTypeIcon, { path: entry.path }), _jsx("span", { children: entry.name }), _jsx("span", { role: "button", tabIndex: 0, className: "hui-tree-at", title: "Reference this file in the input", "aria-label": `Reference ${entry.name} in the input`, onClick: event => { event.stopPropagation(); attachFile(entry.path); }, onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') {
                                                                    event.preventDefault();
                                                                    event.stopPropagation();
                                                                    attachFile(entry.path);
                                                                } }, children: _jsx(AtSign, { size: 13 }) })] }) }, entry.path)), snapshot?.truncated ? _jsx("p", { children: "Too many files \u2014 the list is truncated" }) : null] })] }) : _jsxs(_Fragment, { children: [_jsxs("div", { className: "hui-section-title", role: "button", title: changesOpen ? 'Collapse changes' : 'Expand changes', onClick: () => setChangesOpen(current => !current), children: [changesOpen ? _jsx(ChevronDown, { size: 14 }) : _jsx(ChevronRight, { size: 14 }), _jsx("b", { children: "CHANGES" }), _jsx("em", { children: snapshot?.changes.length ?? 0 })] }), changesOpen ? _jsx("div", { className: "hui-tree", children: snapshot?.gitAvailable === false ? _jsx("div", { className: "hui-empty-small", children: "The current directory is not a Git repository" }) : snapshot?.gitAvailable && snapshot.changes.length === 0 ? _jsx("div", { className: "hui-empty-small", children: "No pending changes" }) : snapshot?.changes.map(change => _jsxs("button", { type: "button", className: "hui-tree-row hui-change", "data-selected": selected === change.path || undefined, title: change.path, onClick: () => open(change.path, 'diff'), children: [_jsx("span", { children: change.path.split('/').at(-1) }), _jsx("small", { children: change.path.includes('/') ? change.path.slice(0, change.path.lastIndexOf('/')) : '' }), _jsx("b", { "data-status": change.status, children: change.status })] }, `${change.status}:${change.path}`)) }) : null, _jsxs("div", { className: "hui-section-title", role: "button", title: historyOpen ? 'Collapse history' : 'Expand history', onClick: () => setHistoryOpen(current => !current), children: [historyOpen ? _jsx(ChevronDown, { size: 14 }) : _jsx(ChevronRight, { size: 14 }), _jsx("b", { children: "HISTORY" }), _jsx("em", { children: gitLog?.length ?? 0 })] }), historyOpen ? _jsx("div", { className: "hui-tree", children: snapshot?.gitAvailable === false ? _jsx("div", { className: "hui-empty-small", children: "The current directory is not a Git repository" }) : gitLog === null ? _jsx("div", { className: "hui-empty-small", children: "Loading history\u2026" }) : gitLog.length === 0 ? _jsx("div", { className: "hui-empty-small", children: "No commits yet" }) : gitLog.map(entry => _jsxs("button", { type: "button", className: "hui-tree-row hui-commit", "data-selected": selected === entry.hash || undefined, title: `${entry.subject}\n${entry.author}\n${new Date(entry.timestamp * 1000).toLocaleString()}`, onClick: () => openCommit(entry.hash), children: [_jsxs("span", { className: "hui-commit-meta", children: [_jsx("b", { children: entry.shortHash }), _jsx("small", { children: commitTime(entry.timestamp) })] }), _jsx("span", { children: entry.subject })] }, entry.hash)) }) : null] })] }), selected !== null && mode !== 'terminal' ? _jsxs("section", { className: "hui-editor", children: [_jsx("div", { className: "hui-editor-tabs", children: _jsxs("div", { className: "hui-editor-tab", children: [_jsx("span", { children: selectedName }), _jsx("button", { type: "button", onClick: closeFile, "aria-label": `Close ${selectedName}`, title: "Close file", children: _jsx(X, { size: 14 }) })] }) }), _jsxs("div", { className: "hui-breadcrumbs", children: [_jsx("div", { children: selected.split('/').map((part, index) => _jsxs("span", { children: [index > 0 ? _jsx(ChevronRight, { size: 10 }) : null, part] }, `${part}:${index}`)) }), _jsx("b", { children: presentation.toUpperCase() })] }), _jsx("div", { className: "hui-editor-surface", "data-presentation": presentation, children: presentation === 'image' && previewDataUrl !== undefined ? _jsxs("div", { className: "hui-image-view", children: [_jsx("img", { src: previewDataUrl, alt: selectedName ?? 'Image preview' }), _jsx("span", { children: selectedName })] }) : presentation === 'markdown' ? _jsx(MarkdownPreview, { content: preview }) : presentation === 'json' ? _jsx(CodePreview, { content: formattedJson(preview), language: hljsLanguageOf(selected ?? '') }) : presentation === 'diff' ? _jsx(CodePreview, { content: preview, diff: true }) : presentation === 'code' ? _jsx(CodePreview, { content: preview, language: hljsLanguageOf(selected ?? '') }) : presentation === 'binary' ? _jsxs("div", { className: "hui-binary-view", children: [_jsx("strong", { children: "This binary file cannot be previewed" }), _jsx("span", { children: selectedName })] }) : _jsx("div", { className: "hui-text-view", children: preview }) })] }) : null] }), _jsxs("footer", { className: "hui-statusbar", children: [_jsxs("span", { children: [_jsx(GitBranch, { size: 12 }), " ", snapshot?.gitAvailable ? `${snapshot.changes.length} changes` : 'Not a Git project'] }), _jsx("span", { children: snapshot?.rootName ?? 'Project' })] })] });
}
...
function HarnessSummary({ wide, sessions }) {
    ...
    return _jsxs("div", { className: `hui-summary${wide ? '' : ' rail'}`, children: [_jsxs("div", { className: "hui-summary-main", "aria-label": "Harness status", title: snapshot?.balance.error, children: [_jsx("span", { className: "hui-icon", children: _jsx(Activity, { size: 13 }) }), wide ? _jsxs("span", { className: "hui-content", children: [_jsxs("span", { className: "hui-card-head", children: [_jsx("span", { className: "hui-card-title", children: "Harness" }), _jsxs("span", { className: "hui-period", "data-period": period, title: period === 'idle' ? 'Off-peak billing period' : 'Peak billing period', children: [_jsx("i", {}), period === 'idle' ? 'Off-peak' : 'Peak'] })] }), _jsxs("span", { className: "hui-balance-line", children: [_jsx("b", { "data-tone": balanceTone, children: balanceValue }), _jsx("small", { children: "Balance" })] })] }) : null] }), _jsx(ProjectDrawer, { sessionId: sessionId, cwd: cwd })] });
}
