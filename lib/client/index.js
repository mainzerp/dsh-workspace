...
function HarnessSummary({ wide, sessions }) {
    ...
    return _jsxs("div", { className: `hui-summary${wide ? '' : ' rail'}`, children: [_jsxs("div", { className: "hui-summary-main", "aria-label": "Harness \u72B6\u6001", title: snapshot?.balance.error, children: [_jsx("span", { className: "hui-icon", children: _jsx(Activity, { size: 12 }) }), wide ? _jsx("span", { className: "hui-content", children: _jsxs("span", { className: "hui-main-row", children: [_jsxs("span", { className: "hui-metrics", children: [_jsxs("span", { children: [_jsx("small", { children: "\u4F59\u989D" }), _jsx("b", { "data-tone": balanceTone, children: balanceValue })] }), _jsxs("span", { children: [_jsx("small", { children: "\u4ECA\u65E5" }), _jsxs("b", { title: cost?.source === 'estimate' ? '官方接口不可用，按本地估算' : 'DeepSeek 平台账单', children: ["\u00A5", cost ? cost.total.toFixed(3) : '—'] })] })] }), _jsxs("span", { className: "hui-period", "data-period": period, children: [_jsx("i", {}), period === 'idle' ? '空闲' : '高峰'] })] }) }) : null] }), _jsx(ProjectDrawer, { sessionId: sessionId, cwd: cwd })] });
}
function HarnessUpdate({ wide }) {
    ...
    if (!wide || currentVersion === null)
        return null;
    return _jsxs(_Fragment, { children: [_jsxs("div", { className: "hui-update-bar", "data-outdated": outdated || undefined, children: [_jsx("span", { children: outdated ? `有新版：v${update.latestVersion}` : `Harness v${currentVersion}` }), outdated ? _jsx("button", { type: "button", className: "hui-update-btn", disabled: updating === 'running', onClick: runUpdate, children: updating === 'running' ? '更新中…' : '更新' }) : null, updating === 'idle' ? null : _jsx("em", { title: updateMessage, children: updateMessage })] }), transitioning ? _jsx("div", { className: "hui-transition", "aria-hidden": "true", children: _jsx("img", { src: FAVICON_SRC, alt: "" }) }) : null] });
}
...
export function apply(ctx) {
    ...
    const Update = ({ wide }) => _jsx(HarnessUpdate, { wide: wide });
