...
function refTooltip(ref) {
    if (ref.kind === 'branch')
        return t.refBranch(ref.name);
    if (ref.kind === 'remote')
        return t.refRemote(ref.name);
    if (ref.kind === 'tag')
        return t.refTag(ref.name);
    return t.refHead;
}
...
function codemirrorLanguageOf(path) {
    const extension = path.split('.').at(-1)?.toLowerCase() ?? '';
    switch (extension) {
        case 'js':
        case 'mjs':
...
function CodeEditor({ path, initialContent, saving, onSave, onCancel }) {
    ...
    const cancel = useCallback(() => {
        const view = viewRef.current;
        if (view !== null && view.state.doc.toString() !== initialContent && !window.confirm(t.editorDiscardChanges))
            return;
        onCancel();
    }, [initialContent, onCancel]);
    return _jsxs("div", { className: "hui-editor-edit", children: [_jsx("div", { className: "hui-editor-cm", ref: containerRef }), _jsxs("div", { className: "hui-editor-actions", children: [_jsx("button", { type: "button", "data-primary": "true", disabled: saving, onClick: save, children: saving ? t.editorSaving : t.editorSave }), _jsx("button", { type: "button", disabled: saving, onClick: cancel, children: t.editorCancel })] })] });
}
...
function ProjectDrawer({ sessionId, cwd }) {
    ...
    const [drawerWidth, setDrawerWidth] = useState(() => (typeof window === 'undefined' ? null : readDrawerWidth(window.innerWidth)));
    ...
    const saveFile = useCallback((content) => {
        if (selected === null)
            return;
        setSaving(true);
        setEditorError(null);
    ...
    const uploadFile = useCallback(async (file, overwrite) => {
        ...
        let binary = '';
        for (let index = 0; index < buffer.length; index += 32_768)
            binary += String.fromCharCode(...buffer.subarray(index, index + 32_768));
        const params = new URLSearchParams();
        if (sessionId !== undefined)
            params.set('sessionId', sessionId);
        if (cwd !== undefined)
    ...
    const expandToView = useCallback((next) => {
        setMode(next);
        writeDrawerCollapsed(false);
        setCollapsed(false);
    }, []);
    return _jsxs("aside", { className: "hui-drawer", "data-collapsed": collapsed || undefined, "aria-label": t.projectPreview, style: effectiveWidth === null ? undefined : { width: effectiveWidth }, children: [collapsed ? null : _jsx("div", { className: "hui-resizer", "aria-hidden": "true", onPointerDown: startResize }), collapsed ? _jsxs("nav", { className: "hui-activity hui-rail", "aria-label": t.projectViews, children: [_jsx("button", { type: "button", "aria-label": t.explorer, title: t.explorer, onClick: () => expandToView('files'), children: _jsx(FolderTree, { size: 20 }) }), _jsxs("button", { type: "button", "aria-label": t.sourceControl, title: t.sourceControl, onClick: () => expandToView('changes'), children: [_jsx(GitBranch, { size: 20 }), snapshot?.changes.length ? _jsx("b", { children: snapshot.changes.length }) : null] }), _jsx("button", { type: "button", "aria-label": t.terminal, title: t.terminal, onClick: () => expandToView('terminal'), children: _jsx(Terminal, { size: 20 }) }), _jsx("button", { type: "button", className: "hui-rail-toggle", "aria-label": t.expandDrawer, title: t.expandDrawer, onClick: () => applyCollapsed(false), children: _jsx(PanelRightOpen, { size: 20 }) })] }) : null, _jsxs("header", { className: "hui-titlebar", children: [_jsxs("div", { children: [_jsx("span", { className: "hui-vscode-mark", children: _jsx(Blocks, { size: 14 }) }), _jsx("span", { title: snapshot?.rootPath, children: snapshot?.rootPath ?? t.loadingCurrentProject })] }), _jsx("button", { type: "button", "aria-label": t.collapseDrawer, title: t.collapseDrawer, onClick: () => applyCollapsed(true), children: _jsx(PanelRightClose, { size: 14 }) })] }), error ? _jsxs("div", { className: "hui-error", children: [error, _jsx("button", { type: "button", onClick: () => setError(null), "aria-label": t.dismiss, title: t.dismiss, children: _jsx(X, { size: 12 }) })] }) : null, notice ? _jsxs("div", { className: "hui-notice", children: [notice, _jsx("button", { type: "button", onClick: () => setNotice(null), "aria-label": t.dismiss, title: t.dismiss, children: _jsx(X, { size: 12 }) })] }) : null, _jsxs("div", { className: "hui-workbench", "data-preview": (selected !== null && mode !== 'terminal') || undefined, children: [_jsxs("nav", { className: "hui-activity", "aria-label": t.projectViews, children: [_jsx("button", { type: "button", "aria-label": t.explorer, title: t.explorer, "data-active": mode === 'files' || undefined, onClick: () => setMode('files'), children: _jsx(FolderTree, { size: 20 }) }), _jsxs("button", { type: "button", "aria-label": t.sourceControl, title: t.sourceControl, "data-active": mode === 'changes' || undefined, onClick: () => setMode('changes'), children: [_jsx(GitBranch, { size: 20 }), snapshot?.changes.length ? _jsx("b", { children: snapshot.changes.length }) : null] }), _jsx("button", { type: "button", "aria-label": t.terminal, title: t.terminal, "data-active": mode === 'terminal' || undefined, onClick: () => setMode('terminal'), children: _jsx(Terminal, { size: 20 }) })] }), _jsxs("section", { className: "hui-explorer", children: [_jsxs("header", { children: [_jsx("strong", { children: mode === 'files' ? t.explorer : mode === 'changes' ? t.sourceControl : t.terminal }), mode !== 'terminal' ? _jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: () => fileInputRef.current?.click(), "aria-label": t.upload, title: t.upload, children: _jsx(Upload, { size: 15 }) }), _jsx("button", { type: "button", onClick: refresh, "aria-label": t.refresh, title: t.refresh, children: _jsx(RefreshCw, { size: 15 }) })] }) : null] }), mode !== 'terminal' ? _jsx("input", { ref: fileInputRef, type: "file", style: { display: 'none' }, onChange: onUploadPick }) : null, mode === 'terminal' ? _jsx(TerminalPanel, { sessionId: sessionId, cwd: cwd }) : mode === 'files' ? _jsxs(_Fragment, { children: [_jsxs("div", { className: "hui-section-title", children: [_jsx(ChevronDown, { size: 14 }), _jsx("b", { children: snapshot?.rootName?.toUpperCase() ?? 'PROJECT' })] }), _jsxs("div", { className: "hui-tree", role: "tree", children: [visibleEntries.map(entry => entry.kind === 'directory'
                                            ...
                                            : gitLog === null ? _jsx("div", { className: "hui-empty-small", children: t.loadingHistory })
                                                : gitLog.length === 0 || graphLayout === null ? _jsx("div", { className: "hui-empty-small", children: t.noCommits })
                                                    : _jsxs("div", { className: "hui-graph", role: "list", "aria-label": t.historyGraph, children: [_jsxs("svg", { className: "hui-graph-edges", width: graphLayout.laneCount * GRAPH_LANE_WIDTH, height: gitLog.length * GRAPH_ROW_HEIGHT, "aria-hidden": "true", children: [graphLayout.edges.map((edge, index) => {
                                                                        ...
                                                                        return edge.from.lane === edge.to.lane
                                                                            ? _jsx("line", { x1: x1, y1: y1, x2: x2, y2: y2, stroke: color, strokeWidth: 1.5 }, index)
                                                                            : _jsx("path", { d: `M ${x1} ${y1} C ${x1} ${y1 + GRAPH_ROW_HEIGHT}, ${x2} ${y2 - GRAPH_ROW_HEIGHT}, ${x2} ${y2}`, fill: "none", stroke: color, strokeWidth: 1.5 }, index);
                                                                    }), graphLayout.nodes.map(node => _jsx("circle", { cx: node.lane * GRAPH_LANE_WIDTH + GRAPH_LANE_WIDTH / 2, cy: node.row * GRAPH_ROW_HEIGHT + GRAPH_ROW_HEIGHT / 2, r: 3.5, fill: LANE_PALETTE[node.lane % LANE_PALETTE.length] }, node.hash))] }), gitLog.map(entry => _jsxs("button", { type: "button", role: "listitem", className: "hui-tree-row hui-commit", "data-selected": selected === entry.hash || undefined, style: { paddingLeft: graphLayout.laneCount * GRAPH_LANE_WIDTH + 8 }, title: `${entry.subject}\n${entry.author}\n${new Date(entry.timestamp * 1000).toLocaleString()}`, onClick: () => openCommit(entry.hash), children: [entry.refs.map(ref => _jsx("span", { className: "hui-ref", "data-kind": ref.kind, title: refTooltip(ref), children: ref.kind === 'head' ? 'HEAD' : ref.name }, `${ref.kind}:${ref.name}`)), _jsx("span", { className: "hui-commit-subject", children: entry.subject }), _jsxs("span", { className: "hui-commit-meta", children: [_jsx("b", { children: entry.shortHash }), _jsx("small", { children: commitTime(entry.timestamp) })] })] }, entry.hash))] }) }) : null] })] }), selected !== null && mode !== 'terminal' ? _jsxs("section", { className: "hui-editor", children: [_jsx("div", { className: "hui-editor-tabs", children: _jsxs("div", { className: "hui-editor-tab", children: [_jsx("span", { children: selectedName }), _jsx("button", { type: "button", onClick: closeFile, "aria-label": t.closeFileName(selectedName ?? ''), title: t.closeFile, children: _jsx(X, { size: 14 }) })] }) }), _jsxs("div", { className: "hui-breadcrumbs", children: [_jsx("div", { children: selected.split('/').map((part, index) => _jsxs("span", { children: [index > 0 ? _jsx(ChevronRight, { size: 10 }) : null, part] }, `${part}:${index}`)) }), editing ? null : editable ? _jsx("button", { type: "button", className: "hui-edit-button", onClick: startEdit, children: t.editorEdit }) : previewSource === 'file' && (truncated || previewBinary) ? _jsx("span", { className: "hui-editor-hint", children: truncated ? t.editorDisabledTruncated : t.editorDisabledBinary }) : null, _jsx("b", { children: presentation.toUpperCase() })] }), _jsx("div", { className: "hui-editor-surface", "data-presentation": presentation, children: editing ? _jsx(CodeEditor, { path: selected, initialContent: draftContent, saving: saving, onSave: saveFile, onCancel: cancelEdit }) : presentation === 'image' && previewDataUrl !== undefined ? _jsxs("div", { className: "hui-image-view", children: [_jsx("img", { src: previewDataUrl, alt: selectedName ?? t.imagePreview }), _jsx("span", { children: selectedName })] }) : presentation === 'json' ? _jsx(CodePreview, { content: formattedJson(preview), language: hljsLanguageOf(selected ?? '') }) : presentation === 'diff' ? _jsx(CodePreview, { content: preview, diff: true }) : presentation === 'code' ? _jsx(CodePreview, { content: preview, language: hljsLanguageOf(selected ?? '') }) : presentation === 'binary' ? _jsxs("div", { className: "hui-binary-view", children: [_jsx("strong", { children: t.binaryNoPreview }), _jsx("span", { children: selectedName })] }) : _jsx("div", { className: "hui-text-view", children: preview }) }), editorError ? _jsx("div", { className: "hui-editor-error", children: editorError }) : null] }) : null] }), _jsxs("footer", { className: "hui-statusbar", children: [_jsxs("span", { children: [_jsx(GitBranch, { size: 12 }), " ", snapshot?.gitAvailable ? t.changesCount(snapshot.changes.length) : t.notGitProject] }), _jsx("span", { children: snapshot?.rootName ?? t.project })] })] });
}
...
export async function apply(ctx) {
    try {
        ...
        if (response.ok)
            setLanguage((await response.json()).language);
    }
    ...
    ctx.effect(() => {
        ...
        style.id = STYLE_ID;
        style.textContent = STYLES + STATUS_STYLES + BASE_STYLES + FLAT_STYLES + EDITOR_STYLES + GRAPH_STYLES;
