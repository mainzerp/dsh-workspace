...
export function relaunchHarness() {
    const entry = process.argv[1];
    if (entry === undefined)
        return;
...
export function runHarnessUpdate() {
    const dir = findDshInstallDir();
    if (dir === null)
        return Promise.resolve({ ok: false, message: '无法定位 dsh 安装目录' });
    ...
    return new Promise(resolvePromise => {
        ...
        child.on('close', code => {
            clearTimeout(timer);
            if (code === 0)
                resolvePromise({ ok: true, message: '更新完成，即将自动重启' });
            else
                resolvePromise({ ok: false, message: `npm 更新失败（exit ${String(code)}）：${output.slice(-300)}` });
        });
    });
}
