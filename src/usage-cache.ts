 ...
 */
export function selectStaleSnapshots<T extends UsageSnapshot>(
  snapshots: readonly T[],
  cache: ReadonlyMap<string, CachedUsage>,
  startTime: number,
): { hits: Map<string, CachedUsage>; stale: T[] } {
  const hits = new Map<string, CachedUsage>()
  const stale: T[] = []
  for (const snapshot of snapshots) {
    const cached = cache.get(snapshot.header.id)
    if (cached !== undefined && cached.revision === snapshot.revision && cached.startTime === startTime) hits.set(snapshot.header.id, cached)
